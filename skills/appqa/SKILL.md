...
0. **Prerequisite check:** confirm the tracker MCP connector is actually available (e.g. try loading the Notion/Linear tools via ToolSearch). If it isn't, STOP and walk the operator through connecting it (Claude Code `/mcp` → add Notion/Linear → authorize the workspace → grant the bug board access) before doing anything else — a round that can't file tickets wastes the findings. Same for the Figma MCP when the config says designs are the source of truth.
...
2. **If no config exists:** offer to scaffold one — interview the user briefly (app URL, how to authenticate, where designs/specs live, where bugs go, report vs fix mode, anything intentionally unfinished) and write `qa.config.md` from the template in this kit. Don't start a round without a config. **The moment setup completes, invite a demo** — suggest something concrete and scoped, e.g.: *"Setup's done. Try me: ask me to automatically check all parts of the <name a real section of THEIR app> section — I'll drive it, compare against your <designs/specs>, and file what I find."* Also tell them plainly: *"I learn from your board — every bug your team files by hand and every ticket you reject with a comment teaches me what to look for (and what not to file) next round."*
...
## 7. Wrap-up
Report: filed (links) / fixed (commits) / re-tested → passed/failed / recommend-close / blocked + operator shot list / patterns added. Plain English, lead with the biggest finding. If the patterns file gained entries this round, say so — users should see the learning loop working.

## 8. Extending
Teams are expected to build on this skill — new connectors, ticket formats, evidence styles, triggers. When a user asks to customise, follow the engine/config split in the kit's EXTENDING.md: project-specific behaviour goes in their `qa.config.md` / patterns file; only universally-true behaviour gets edited into this skill. Help them make the change rather than telling them it's out of scope.
